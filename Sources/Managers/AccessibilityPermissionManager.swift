...
internal class AccessibilityPermissionManager {
    ...
    private func showPermissionGrantedConfirmation() {
        ...
        You can disable SmartPaste anytime in SpeedyWhisper's Settings if you prefer manual control.
        """
        alert.alertStyle = .informational
    ...
    private func showPermissionTimeoutMessage() {
        ...
        • Restart SpeedyWhisper if the permission still doesn't work
    ...
    private func showAccessibilityPermissionEducation() {
        ...
        Task { @MainActor in
            ...
            🔍 Why SpeedyWhisper Needs This Permission:
            
            SpeedyWhisper needs to send a simple "paste" command (equivalent to pressing ⌘V) to place transcribed text in the right location. Without this permission, you'd need to manually:
            ...
            • SpeedyWhisper is sandboxed and can't access other app's data
    ...
    func showManualPermissionInstructions() {
        Task { @MainActor in
            ...
            3. Find SpeedyWhisper in the list
            4. Toggle the switch to enable it
            5. Return to SpeedyWhisper
            
            If SpeedyWhisper isn't in the list, you may need to add it manually using the "+" button.
            
            💡 Troubleshooting:
            • If the toggle appears grayed out, click the lock icon and enter your password
            • If SpeedyWhisper doesn't appear in the list, try restarting SpeedyWhisper
            • You may need to remove and re-add SpeedyWhisper if it's not working
            """
            alert.alertStyle = .informational
    ...
    var detailedPermissionStatus: (isGranted: Bool, statusMessage: String, troubleshootingInfo: String?) {
        ...
        if isGranted {
            ...
            return (
                ...
                2. Add SpeedyWhisper to the list (using + button if needed)
                3. Toggle the switch to enable SpeedyWhisper
                4. Restart SpeedyWhisper if needed
                """
            )
        }
    }
    ...
    func handlePermissionError(_ error: Error) {
        Task { @MainActor in
            ...
            2. Go to Privacy & Security → Accessibility
            3. Add SpeedyWhisper and enable it
            
            Or continue using SpeedyWhisper without SmartPaste - transcribed text will be copied to your clipboard for manual pasting.
            """
            alert.alertStyle = .warning
            alert.addButton(withTitle: "Open System Settings")
            alert.addButton(withTitle: "Continue Without SmartPaste")
    ...
    func showPermissionDeniedMessage() {
        Task { @MainActor in
            ...
            alert.informativeText = """
            SpeedyWhisper will continue to work without SmartPaste functionality.
            
            Transcribed text will be copied to your clipboard, and you can paste it manually using ⌘V.
            
            You can enable SmartPaste anytime in SpeedyWhisper Settings → General → Accessibility Permissions.
            """
            alert.alertStyle = .informational
            alert.addButton(withTitle: "OK")
            alert.runModal()
